...
Livewireコンポーネントは、他の多くのコンポーネントベースのフレームワークと同様に、入れ子（ネスト）にできます。つまり、ひとつのコンポーネントの中で複数のコンポーネントをレンダリングできます。

ただし、Livewireのネストシステムは他のフレームワークとは異なる仕組みで構築されているため、いくつか注意すべき点や制約があります。

:::tip まずは「ハイドレート」について理解しましょう
Synthesizerを使う前に、Livewireのハイドレートシステムを理解しておくと役立ちます。詳しくは[ハイドレートのドキュメント](/docs/hydration)をご覧ください。
:::
...
投稿上限が「1」になったため、子コンポーネントは1つだけレンダリングされています。しかし、完全な子コンポーネントの内容ではなく、対応する`wire:id`属性を持つ空の`<div></div>`だけが出力されていることに注目してください。

このHTMLがフロントエンドで受信されると、Livewireはこのコンポーネントの古いHTMLを新しいHTMLに_morph_（変形）しますが、子コンポーネントのプレースホルダーは賢くスキップします。

その結果、_morphing_後の親`Posts`コンポーネントの最終的なDOM内容は次のようになります：
