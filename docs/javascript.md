...
ブラウザでLivewireコンポーネントにアクションが実行されると、ネットワークリクエストがトリガーされます。そのネットワークリクエストには、ひとつまたは複数のコンポーネントとサーバーへのさまざまな指示が含まれています。内部的に、これらのコンポーネントネットワークペイロードは"コミット"と呼ばれます。
